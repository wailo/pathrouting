  ...
  if ( node->type == Node::NODE_TYPE ) {
	...
	for ( unsigned int i=0 ; i<4 ; ++i )
	{
		if ( node->Child[i] == NULL ) // To prevent creating duplicate nodes
		{
			...
			if ( node->Child[i]->depth > maxGridDepth ) { maxGridDepth = node->Child[i]->depth; }
			node->Child[i]->type = Node::LEAF_TYPE;
			node->Child[i]->id = ( 4 * node->Child[i]->Parent->id ) - 2 + i;
			if ( node->type != Node::ROOT_TYPE ) { node->type = Node::NODE_TYPE; }
		}
...
void QuadTree::getAllNeighbours ( Node *thisNode , std::vector< Node*>& vector)
{
	...
	for (int Direction=NW; Direction<= W; ++Direction )
	{
		...
		case E:
			...
			// Get Eastern node
			pNeighbourNode = findTreeNode( thisNode->centre_x() +  ( thisNode->x_dsp() + (thisNode->x_dsp( maxGridDepth ) / 2.0) ) , thisNode->centre_y() );
			...
			if ( pNeighbourNode->id != thisNode->id )
			{
				...
				// Now find all the NW and SW children of the neighbouring node that share the same edge with this node and add the to a container
		...
		case W:
			...
			if ( pNeighbourNode->id != thisNode->id )
			{
				while (pNeighbourNode->depth > thisNode->depth)	{ pNeighbourNode = pNeighbourNode->Parent; }
		...
		case N:
			pNeighbourNode = findTreeNode( thisNode->centre_x(), thisNode->centre_y() -  ( thisNode->y_dsp() + (thisNode->y_dsp( maxGridDepth ) / 2.0) ) + 0.005);
		...
		case S:
			pNeighbourNode = findTreeNode( thisNode->centre_x(), thisNode->centre_y() +  ( thisNode->y_dsp() + (thisNode->y_dsp( maxGridDepth ) / 2.0) ));
			if ( pNeighbourNode->id != thisNode->id )
			{
				while (pNeighbourNode->depth > thisNode->depth)	{ pNeighbourNode = pNeighbourNode->Parent; }
		...
		case NW:
			pNeighbourNode = findTreeNode( thisNode->centre_x() -  ( thisNode->x_dsp() + (thisNode->x_dsp( maxGridDepth ) / 2.0)), thisNode->centre_y() - ( thisNode->y_dsp() + (thisNode->y_dsp( maxGridDepth ) / 2.0) ));
			if ( pNeighbourNode->id != thisNode->id )
			{
...
Node* QuadTree::findNeighbour ( Node *p, char Direction)
{
	...
	for ( int i=0; i<xbit.size(); i++ ) {qDebug().maybeSpace() << xbit[i];}
